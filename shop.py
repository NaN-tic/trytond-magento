...
class SaleShop:
    ...
    def view_attributes(cls):
        return super(SaleShop, cls).view_attributes() + [
            ('/form/notebook/page[@id="esale"]/notebook/page[@id="magento"]',
                'states', {
    ...
    def mgn2order_values(self, values):
        ...
        if (values.get('discount_amount') and
            ...
            if self.esale_tax_include:
                customer_taxes = (
                    self.esale_discount_product.template.customer_taxes_used)
                # TODO support multiple taxes
                rate = customer_taxes[0].rate if customer_taxes else 0
                external_discount = base_price_without_tax(
                    external_discount, rate, self.currency)
    ...
    def mgn2lines_values(self, values, party_values, sales_values):
        '''
        ...
        def is_discount_in_line(self, item, discount_type):
            """
            :param item: dict magento values
            :param discount_type: discount_percent or discount_amount
            """
            if (not self.esale_discount_new_line
                    and hasattr(SaleLine, 'gross_unit_price')
                    and item.get(discount_type)
                    and item.get(discount_type) != '0.0000'):
                return True
            else:
                return False
        ...
        for item in values.get('items'):
            ...
            if item['product_type'] not in PRODUCT_TYPE_OUT_ORDER_LINE:
                ...
                gross_unit_price = Decimal(item.get('base_price'))
                ...
                if is_discount_in_line(self, item, 'base_discount_amount'):
                    discount_amount = Decimal(item['base_discount_amount'])
                    if self.esale_tax_include:
                    ...
                    discount_amount_aux = discount_amount
                    discount_amount = discount_amount / qty
                    discount_percent = discount_amount / gross_unit_price
                elif is_discount_in_line(self, item, 'discount_percent'):
                    discount_percent = Decimal(item['discount_percent']) / 100
                    if discount_percent == 1:
                        discount_percent = 0
                        sales_values['comment'] += ('\n\n"NOT APPLYED DISCOUNT '
                            '100% ON PRODUCT %s"' % (
                                item['sku'].split('-')[-1]))

                # Product Options (available feature with product simple)
                if app.product_options and item.get('sku'):
                    line = Product.magento_product_type_simple(
                        app, item, gross_unit_price, product, sequence)
                    line['product'] = item['sku'].split('-')[-1]
                else:
                    # Get Product Type Attribute to transform data
                    method_type = 'magento_product_type_%s' % item.get(
                        'product_type')
                    if hasattr(Product, method_type):
                        product_type = getattr(Product, method_type)
                    else:
                        product_type = getattr(
                            Product, 'magento_product_type_simple')
                    line = product_type(app, item, gross_unit_price, product,
                        sequence)
                    price = gross_unit_price * (1 - discount_percent)
    ...
    def mgn2party_values(self, values):
        ...
        if country_id:
            ...
            if countries:
                ...
                tax_rule = eSaleAccountTaxRule.compute(country, subdivision,
                    zip)
                if tax_rule:
    ...
    def import_orders_magento(self, ofilter=None):
        ...
        if not ofilter:
            ...
            # filter orders by store views. Get all views from a website
            # related a shop
            store_views = []
        ...
        with magento.Order(mgnapp.uri, mgnapp.username, mgnapp.password) as \
                order_api:
            try:
        ...
        with Transaction().set_context(context):
            for grouped_order_ids in grouped_slice(order_ids, MAX_CONNECTIONS):
                with magento.Order(mgnapp.uri, mgnapp.username,
                        mgnapp.password) as order_api:
